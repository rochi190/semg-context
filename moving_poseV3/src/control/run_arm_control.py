...
★ 分類器程式碼與已訓練模型來自 `semg_realtime_V6/`（repo 根目錄下的自足打包，
跟 `moving_pose/` 同層，不在它底下），不是 `src/semg/`（那是舊快照）。
`--model` 預設指到 `semg_realtime_V6/models/final_v4_split.pt`——split 架構
...
# semg_realtime_V6/src 插在最前面，蓋過 src/semg 的舊快照——真正在用的分類器
# 程式碼與已訓練模型都在 semg_realtime_V6/（見 arm_config.py 的說明）。
sys.path.insert(0, str(AC.SEMG_REALTIME_SRC))
# 必須在這裡呼叫、不能在 arm_config.py 檔尾自動呼叫——validate() 的時序一致性
# 檢查要 import semg.v2.config，得等 semg_realtime_V6 路徑插好之後才能撿到
...
def main() -> None:
    ...
    ap.add_argument("--model", type=Path, default=AC.DEFAULT_MODEL,
                    help="--source replay/live 用：模型 bundle（預設 semg_realtime_V6 的 "
    ...
    ap.add_argument("--dwell", type=int, default=AC.DEFAULT_DWELL,
                    help="--source replay/live 用：候選新狀態要連續出現幾次才真的換過去，"
                         "擋「移動過程被穩定地標成錯的狀態」（見 semg_realtime_V6/README.md）")
    # ── 環境雜訊抑制（2026-09-12 加入）────────────────────────────
    # ★ 手臂在實驗室（電器多、雜訊大），但訓練資料錄在安靜的地方。
    #   這些演算法把吵環境的訊號映射回乾淨環境的樣子。
    #   每個演算法的實測效果見 docs/denoise.md —— ⚠️ 不是每個都該開。
    ap.add_argument("--denoise", default=None, metavar="a,b,c",
                    help="降噪演算法，逗號分隔：comb / lms / car / hampel / wavelet")
    for _a, _h in (("comb", "60Hz 諧波梳狀陷波（120/180/…/420Hz）"),
                   ("lms", "自適應市電消除（不挖頻譜洞、會追漂移）"),
                   ("car", "分組共同平均參考 ⚠️ 乾淨訊號上實測 −15pp，不建議"),
                   ("hampel", "突波抑制"),
                   ("wavelet", "小波軟閾值（實驗性，最貴）")):
    ...
    import sys as _sys
    _sys.path.insert(0, str(AC.SEMG_REALTIME_SRC))
    from semg.v2.denoise import parse_flags, ALGOS
    _dn = parse_flags(a.denoise)
    for _a in ALGOS:
        if getattr(a, _a, False):
            _dn[_a] = True
    if _dn or a.nf:
        print(f"降噪   : {', '.join(sorted(_dn)) or '（無）'}"
