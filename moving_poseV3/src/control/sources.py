...
★ `Recognizer` 與 `config` 實際載入的是 `semg_realtime_V6/` 這個自足打包
（已訓練模型 + 修正過 ch2/ch4 標籤的程式碼），不是 `src/semg/` 舊快照——
由呼叫端（`run_arm_control.py`）在 import 這個模組之前把
`semg_realtime_V6/src` 插到 `sys.path` 最前面決定，這裡不用管路徑。
...
class ReplaySource(PredictionSource):
    ...
    def __init__(self, csv_path: Path, model_path: Path, calib_sec: float = 5.0,
                ...
                denoise: dict | None = None, noise_floor: bool = False):
        ...
        #   （見 semg_realtime_V6/README.md「移動過程的誤判」）。詳見 arm_config.py。
        # ★ 環境降噪（2026-09-12）：手臂在吵的實驗室，訓練資料錄在安靜的地方。
        #   降噪的目標是把吵環境映射回乾淨環境，讓乾淨資料訓練的模型仍適用。
        #   ⚠️ 只有通過「乾淨訊號中性測試」的演算法才該開，見 docs/denoise.md。
        self.rec = Recognizer(model_path, votes=AC.RECOGNIZER_VOTES,
                              conf=AC.RECOGNIZER_CONF, dwell=dwell,
                              denoise=denoise or None, noise_floor=noise_floor)
