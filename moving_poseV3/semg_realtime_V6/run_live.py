...
純螢幕（不接手臂）：
    ...
    python run_live.py COM10 --model final_v5_split    換回 V5 的模型比較

★ 環境降噪（V3 新增）—— 手臂在吵的實驗室，訓練資料錄在安靜的地方：
    python run_live.py COM10 --lms                 ★ 建議：自適應市電消除
    python run_live.py COM10 --lms --nf            ★ 加雜訊底噪補償
    python run_live.py COM10 --denoise comb,lms    諧波梳狀陷波 + LMS
    ⚠️ --car / --wavelet / --hampel 在乾淨訊號上實測會**大幅變差**
       （−15 / −34.5 / −35.6 pp），留著只為了讓你自己驗證。
       各演算法的選擇理由與實測數字見 docs/denoise.md。
