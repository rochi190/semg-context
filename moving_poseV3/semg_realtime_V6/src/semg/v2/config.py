...
# 是振幅的兩個極端。
# ★ V5：要不要多一個 `moving` 自由度（模型明確說「我正在轉換中」）。
#
#   為什麼需要：協定把移動過程整段丟棄，所以模型對「正在移動」沒有概念。
#   實測 v4 重播一份**訓練過**的錄音：有標籤的段（go+release）三個 DOF
#   全對 **100.0%**，但那只佔螢幕時間的 34% —— 另外 66% 是模型從沒訓練過的
#   preview/prepare/return/margin，它只能從學過的狀態裡挑一個輸出，看起來就是亂跳。
#
#   ⚠️ **只有 `return` 能當「移動中」的訓練資料，`prepare` 不行。**
#      這兩段不對稱，是量出來的：
#        prepare 2.0s：到位時間中位 0.40s、p90 0.61s
#                      → 後面約 1.4 秒**已經維持住目標姿勢**，標成 moving 大部分是錯的
#        return  2.5s：訊號衰減 90% 需中位 1.80s、p90 2.86s
#                      → 大部分真的在轉換
#      硬把 prepare 標成 moving 會注入大量錯誤標籤。
#
#   ⚠️ 開啟後 DOF_N_STATES 從 (4,2,2) 變成 (4,2,2,2)，**v4 的模型全部不相容**。
#   ⚠️ 用**環境變數**控制而不是改檔案：多個實驗並行時改檔案會互相污染，
#      而且改了也不影響已經 import 的 process，出錯時極難追。
#          SEMG_MOVING_DOF=1 python -m semg.v2.xxx   → 開啟（V5）
#          （不設）                                  → 關閉（V4 行為，預設）
USE_MOVING_DOF = _os.environ.get("SEMG_MOVING_DOF", "0") == "1"

DOFS = ("hand", "elbow", "shoulder") + (("moving",) if USE_MOVING_DOF else ())
DOF_STATES = {
...
if USE_MOVING_DOF:
    # steady = 正在維持某個姿勢（或休息）；moving = 正在從一個姿勢換到另一個
    DOF_STATES["moving"] = ("steady", "moving")
    # ★ `moving` 是唯一該看**兩組通道**的頭 —— 轉換同時牽涉手與手臂，
    #   而且它的用途正是「告訴控制層現在整體不穩定」，不屬於任何單一體段。
    #   （`DOF_CHANNEL_GROUP` 定義在前面，這裡補上這一項。）
    DOF_CHANNEL_GROUP["moving"] = "both"
DOF_LABELS_ZH = {"hand": "手部", "elbow": "手肘", "shoulder": "肩膀",
                 "moving": "轉換中"}
STATE_LABELS_ZH = {"rest": "中性放鬆", "flex": "屈曲維持",
                   "index": "食指彎", "thumb": "拇指彎", "pinch": "捏住",
                   "steady": "維持中", "moving": "轉換中"}
N_DOF = len(DOFS)
...
def action_to_dof(action: str) -> tuple[int, ...]:
    """動作名 → 每個 DOF 的狀態索引。未提及的 DOF 取**該 DOF 的第一個狀態**。

    ⚠️ 預設不能寫死 `"rest"` —— `moving` 這個 DOF 的狀態是 `(steady, moving)`，
       沒有 `rest`，寫死會 ValueError。每個 DOF 的索引 0 都是它的中性狀態
       （hand/elbow/shoulder 是 rest、moving 是 steady），所以取 [0] 才通用。
    """
    spec = ACTIONS.get(action, {})
    return tuple(DOF_STATES[d].index(spec[d]) if d in spec else 0 for d in DOFS)
