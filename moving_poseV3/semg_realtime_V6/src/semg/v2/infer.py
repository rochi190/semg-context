...
class Recognizer:
    ...
    def __init__(self, bundle: Path, votes: int = 5, conf=0.6,
                 ...
                 dwell=0, bias=0.0, denoise: dict | None = None,
        ...
        # ★ 降噪鏈是**部署期**的選項，不寫進 bundle。
        #   與 use_hampel 的差別：use_hampel 改變「乾淨訊號」的樣子，
        #   所以必須跟訓練一致；降噪的設計目標是把「吵環境」映射回
        #   「乾淨環境」，也就是**用乾淨資料訓練的模型 + 部署時開降噪**
        #   才是預期用法。前提是每個演算法都通過乾淨訊號中性測試。
        self.denoise = dict(denoise or {})
        self.noise_floor = bool(noise_floor)
        self.nf_rms = None                       # 校準時估出的雜訊底噪（mV）
        self.filt = StreamFilter(use_hampel=self.use_hampel,
                                 denoise=self.denoise or None)
    ...
    def calibrate(self, raw_block: np.ndarray, n_tail: int | None = None) -> np.ndarray:
        ...
        if self.noise_floor:
            # ★ 雜訊底噪補償（--nf）
            #
            #   問題：校準尺度 α 是從休息段估的，而休息段在吵的環境下
            #   **包含環境雜訊**。α 被墊高 → 所有 `*_rel` 振幅特徵被壓扁
            #   → 該自由度看起來永遠接近 rest（使用者回報過「shoulder
            #   完全不觸發」就是這個機制）。
            #
            #   做法：從 350–450Hz（幾乎沒有肌電）估寬頻雜訊底，
            #   在**功率域**把它從 α² 扣掉：
            #       α_corrected = sqrt(max(α² − σ_noise², (0.3α)²))
            #   下限 0.3α 是防呆 —— 若估到的雜訊比 α 還大（估錯或
            #   訊號真的全是雜訊），不能讓 α 掉到 0 導致特徵爆炸。
            from semg.v2.denoise import noise_floor_rms
            self.nf_rms = noise_floor_rms(f)
            corrected = np.sqrt(np.maximum(self.scales ** 2 - self.nf_rms ** 2,
                                           (0.3 * self.scales) ** 2))
            self.scales = corrected
            self.epss = 0.05 * self.scales
