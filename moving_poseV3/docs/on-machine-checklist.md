...
> `semg_realtime_V6/run_live.py`，加 `--arm-ip`／`--arm-dry-run` 即驅動手臂，
...
| 取樣率 | 2000 SPS，`CONFIG1=0x93` | 以 `semg_realtime_V6/src/semg/v2/config.py` 為準 |
| Gain | 8（`CHnSET=0x40`），1 LSB ≈ 67.06 nV | 舊文件寫 Gain=4 / 134 nV 是錯的 |
| 分類器套件位置 | **`semg_realtime_V6/`**，跟 `moving_pose/` 同層（repo 根目錄），不在它底下 | 2026-08-19 從 `moving_pose/semg_realtime/`（V3）遷移，見 `arm_config.py` 開頭說明 |
| 預設模型 | `semg_realtime_V6/models/final_v4_split.pt` | split 架構，「沒 cue 過的組合」準確率 5.0%→85.4%（直接解決抬肘同時彎食指被誤判成 pinch 的問題） |
...
`semg_realtime_V6/models/final_v4_split.pt` 就是用修正後的電極標籤、
...
| — | `dof_model` 重訓（ch2/ch4 + split 架構） | `final_v4_split.pt` | 2026-08-19 遷移到 semg_realtime_V6 | 0-3 | ✅ |
...
| `config.ACTUATOR_SEMANTICS` 與控制層不一致（index/thumb 已改配 J6） | `semg_realtime_V6/src/semg/v2/config.py` | 低（僅註解，不影響執行） |
