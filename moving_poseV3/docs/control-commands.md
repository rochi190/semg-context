...
> `CLAUDE_CODE_arm_control_spec.md`、`semg_realtime_V6/README.md`）需要指令時
> 一律連來這裡，不要各自複製一份。設計理由、參數調整依據等**非指令**內容
> 仍留在原本的文件。
>
> 相對路徑的基準：`run_live.py` 從 `moving_poseV2/semg_realtime_V6/` 執行，
...
| 進入點 | 位置 | 訊號來源 | 用途 |
|---|---|---|---|
| **`run_live.py`** | `semg_realtime_V6/` | `--replay <csv>`（讀檔）或 `COM10`（真串列） | 分類 → 螢幕，可選擇同時驅動手臂 |
...
| **模型／特徵／辨識過濾** | `semg_realtime_V6/src/semg/v2/` | 訊號與模型層 |
| **只改終端顯示或資料來源** | `semg_realtime_V6/src/semg/v2/live.py` | 純顯示層 |
...
- 模型架構、電極位置、切窗方式：`semg_realtime_V6/README.md`
