...
class MultiVoter:
    ...
    def __init__(self, dof_n_states, votes: int = 5, conf: float = 0.6,
                 dwell: int = 0):
        self.n_states = list(dof_n_states)
        self.votes, self.conf, self.dwell = votes, conf, int(dwell)
    ...
    def push(self, probs_list) -> list[int | None]:
        for j, p in enumerate(probs_list):
            ...
            if arr[:, cls].mean() < self.conf:
                continue
            if self.dwell <= 0 or cls == self.stable[j]:
                self.stable[j] = cls
...
class Recognizer:
    ...
    def __init__(self, bundle: Path, votes: int = 5, conf: float = 0.6,
