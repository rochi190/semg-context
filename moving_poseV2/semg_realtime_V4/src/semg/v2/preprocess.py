    ...
    def __call__(self, block: np.ndarray) -> np.ndarray:
        ...
        for si, (b, a) in enumerate(self.stages):
            ...
            for c in range(x.shape[1]):
                ...
                y, self.zi[si][c] = lfilter(b, a, x[:, c], zi=self.zi[si][c])
                out[:, c] = y
            x = out
        self._primed = True
...
def filter_causal(x: np.ndarray, use_hampel: bool | None = None) -> np.ndarray:
    """(n, ch) → (n, ch)。離線版 = 把整段餵給 StreamFilter。

    刻意**不另外實作**：只要有第二份實作，就會有第二種行為。
    """
    return StreamFilter(n_ch=x.shape[1], use_hampel=use_hampel)(x)
