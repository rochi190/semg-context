...
class SplitMultiHead(nn.Module):
    ...
    def __init__(self, n_features: int,
        ...
        # "both" 不是一個真的通道分組 —— 它表示該頭要吃**兩個編碼器串接**的表示
        self.groups = sorted({g for d in self.dof_names
                              if (g := C.DOF_CHANNEL_GROUP[d]) != "both"})
        ...
        self.heads = nn.ModuleList([
            nn.Linear(width * (len(self.groups) if g == "both" else 1), n)
            for g, n in zip(self._dof_group, self.dof_n_states)])
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        cat = torch.cat([pooled[g] for g in self.groups], dim=1)
        return [head(cat if g == "both" else pooled[g])
                for head, g in zip(self.heads, self._dof_group)]
...
class SplitZhaoMultiHead(nn.Module):
    """★ Zhao et al. 的 building block（CNN→BiLSTM→Attention→FC），但**分成兩路**。

    ## 為什麼做這個

    `SplitMultiHead` 證實了「分離編碼器」是解決組合泛化的關鍵
    （留一組合 5.0% → 85.4%，3 種子 p=0.0008）。
    但那用的是自建的 SepConv backbone。

    如果**分離**才是關鍵、而不是**用哪種 backbone**，
    那把 Zhao 的架構也拆成兩路，應該同時拿到：
      - 論文架構（寫論文時不用解釋為什麼自創）
      - 組合泛化能力

    這是可證偽的預測 —— 若 split-zhao 的組合泛化仍然很差，
    就代表關鍵不只是分離，BiLSTM 的序列建模本身也在背共現結構。

    ⚠️ 參數量會**很大**（每路一個 BiLSTM(128)），約 zhao 的 1.6 倍。
       所以這個架構是為了回答問題，不一定是要部署的東西。
    """
    ...
    def __init__(self, n_features: int,
        ...
        for g in self.groups:
            ...
            self.register_buffer(f"mask_{g}", torch.as_tensor(m), persistent=True)
            cnn[g] = nn.Sequential(
                nn.Conv1d(int(m.sum()), cnn_filters, kernel, padding=kernel // 2),
                nn.BatchNorm1d(cnn_filters), nn.ReLU())
            lstm[g] = nn.LSTM(cnn_filters, lstm_units, num_layers=1,
                              batch_first=True, bidirectional=True)
            ap[g] = nn.Linear(lstm_units * 2, lstm_units * 2)
            asc[g] = nn.Linear(lstm_units * 2, 1, bias=False)
            fc[g] = nn.Sequential(nn.Linear(lstm_units * 2, fc_units),
                                  nn.ReLU(), nn.Dropout(fc_dropout))
        self.cnn, self.lstm = nn.ModuleDict(cnn), nn.ModuleDict(lstm)
        self.attn_proj, self.attn_score = nn.ModuleDict(ap), nn.ModuleDict(asc)
        self.fc = nn.ModuleDict(fc)
        self.lstm_drop = nn.Dropout(lstm_dropout)
        self._dof_group = [C.DOF_CHANNEL_GROUP[d] for d in self.dof_names]
        self.heads = nn.ModuleList([
            nn.Linear(fc_units * (len(self.groups) if g == "both" else 1), n)
            for g, n in zip(self._dof_group, self.dof_n_states)])
    ...
    def forward(self, x: torch.Tensor) -> list[torch.Tensor]:
        ...
        for g in self.groups:
            m = getattr(self, f"mask_{g}")
            h = self.cnn[g](x[:, :, m].transpose(1, 2)).transpose(1, 2)
            h, _ = self.lstm[g](h)
            h = self.lstm_drop(h)
            a = torch.softmax(self.attn_score[g](torch.tanh(self.attn_proj[g](h))), dim=1)
            z[g] = self.fc[g]((h * a).sum(dim=1))
        cat = torch.cat([z[g] for g in self.groups], dim=1)
        return [head(cat if g == "both" else z[g])
                for head, g in zip(self.heads, self._dof_group)]
...
ARCHITECTURES["split_zhao"] = SplitZhaoMultiHead
