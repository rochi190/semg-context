...
# ---------------- 分類器來源套件（semg_realtime_V4）----------------
# 分類器程式碼與已訓練模型都在 semg_realtime_V4/ 這個自足打包裡，control 層
# 一律從這裡載入 `semg.v2.*` / `hardware.record_session`——兩個 CLI 進入點都會
# 把 SEMG_REALTIME_SRC 插在 sys.path 最前面。見 semg_realtime_V4/README.md。
#
...
SEMG_REALTIME_ROOT = REPO_ROOT / "semg_realtime_V4"
...
# 所以預設用它。見 semg_realtime_V4/README.md「v4 相對 v3 改了什麼」。
DEFAULT_MODEL = SEMG_REALTIME_ROOT / "models" / "final_v4_split.pt"

# ---------------- Recognizer（semg_realtime_V4 的 MultiVoter）----------------
...
# ---------------- 分類器輸出節奏 ----------------
# ★ 這是控制層自己的參數副本，刻意與 semg_realtime_V4/src/semg/v2/config.py
...
#    若你調整了 semg_realtime_V4 那邊的 STEP_SEC（模型輸出頻率），這裡必須
...
#    STEP_SEC，但 semg_realtime_V4/src 是由 run_arm_control.py／
...
#    semg_realtime_V4，檢查結果沒有意義。所以呼叫端必須在把
...
def validate() -> None:
    ...
    # 必須保證這裡 import 到的是 semg_realtime_V4（見上方檔案層級的說明）。
