...
def intervals(cues: pd.DataFrame, n_samples: int,
    ...
    for i, r in rows.iterrows():
        ...
        elif ev == "return" and C.USE_MOVING_DOF:
            # ★ V5：`return` 是唯一標籤可靠的「移動中」段落。
            #
            #   為什麼只有 return、不含 prepare —— 兩段的實測性質完全不同：
            #     prepare 2.0s：到位時間中位 0.40s、p90 0.61s
            #                   → 後面約 1.4 秒**已經維持住目標姿勢**了，
            #                     標成 moving 大部分是錯的
            #     return  2.5s：訊號衰減 90% 需中位 1.80s、p90 2.86s
            #                   → 大部分真的在轉換
            #   硬把 prepare 也標進來 = 注入大量錯誤標籤。
            #
            #   ⚠️ 其餘三個自由度在這段是**未定義**的（回程的姿勢語意模糊），
            #      所以標 -1，靠 CrossEntropyLoss(ignore_index=-1) 遮掉它們的損失。
            #      只有 moving 頭會從這些視窗學東西。
            dof = tuple([-1] * (len(C.DOFS) - 1) + [1])
            out.append({"start": s + ms, "end": e - me, "gesture": "moving",
                        "dof": dof, "phase": "transition",
                        "rep": int(r.get("rep", -1))})
...
def window_label(lab: np.ndarray, start: int, end: int,
    ...
    if len(seg) == 0:
        return None
    # ★ V5：一列只要**有任何一個** DOF 有定義就算有效。
    #   `return` 段的標籤是 (-1,-1,-1,1) —— 三個姿勢頭未定義、moving 頭定義為 1。
    #   舊的 `.all(axis=1)` 會把整段丟掉，那正是 v4 學不到「移動中」的原因。
    #   仍然要求整個視窗的組合**完全一致**，所以跨相位邊界的視窗照樣被丟掉。
    valid = seg[(seg >= 0).any(axis=1)]
    if len(valid) == 0:
        return None
...
def dof_to_action(dof: np.ndarray) -> str:
    ...
    if t == C.REST_DOF:
        return "rest"
    # ⚠️ v 可能是 -1（該 DOF 對這個視窗**未定義**，見 V5 的 return 段）。
    #    直接拿去索引會變成 Python 的負索引 → 取到最後一個狀態，
    #    日誌裡就會出現 `hand_pinch+...` 這種完全錯誤的名稱。
    parts = [f"{d}_{C.DOF_STATES[d][v]}" for d, v in zip(C.DOFS, t)
             if v >= 0 and C.DOF_STATES[d][v] not in ("rest", "steady")]
    if any(v < 0 for v in t):
        return "+".join(parts) + "(其餘未定義)" if parts else "未定義"
